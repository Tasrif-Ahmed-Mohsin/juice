...
class TwoGoApp {
    ...
    showNotification(message, type = 'info', duration = 4000) {
        ...
        // Make messages more concise for mobile
        let shortMessage = message;
        if (message.length > 40) {
            shortMessage = message.substring(0, 37) + '...';
        }

        notification.innerHTML = `
            <div style="display: flex; align-items: center; gap: 0.5rem;">
                <i class="fas fa-${icons[type]}" style="font-size: 0.9rem; opacity: 0.9;"></i>
                <span style="flex: 1; line-height: 1.3; font-size: 0.85rem;">${shortMessage}</span>
                <button onclick="this.parentElement.parentElement.remove()" style="background: none; border: none; color: inherit; cursor: pointer; padding: 0.25rem; opacity: 0.7; transition: opacity 0.2s;">
...
document.addEventListener('DOMContentLoaded', () => {
  const fruityBg = document.querySelector('.fruity-bg');
  if (!fruityBg) return;

  const fruits = Array.from(fruityBg.querySelectorAll('.fruit'));
  const fruitSize = 48; // px
  const threshold = 70; // px
  // Animate fruit movement with left/top and a pop
  fruits.forEach(fruit => {
    fruit.style.transition = 'left 0.9s cubic-bezier(0.16,1,0.3,1), top 0.9s cubic-bezier(0.16,1,0.3,1), transform 0.3s';
    // Ensure the fruit is absolutely positioned with left/top set in CSS
    // If fruit doesn't have inline left/top, set them from computed style
    if (!fruit.style.left) {
      fruit.style.left = window.getComputedStyle(fruit).left;
    }
    if (!fruit.style.top) {
      fruit.style.top = window.getComputedStyle(fruit).top;
    }
  });

  fruityBg.addEventListener('mousemove', (e) => {
    const mouseX = e.clientX;
    const mouseY = e.clientY;
    fruits.forEach(fruit => {
      // Only trigger once per fruit until it moves away
      if (fruit.dataset.runaway === 'true') return;
      const rect = fruit.getBoundingClientRect();
      const fruitCenterX = rect.left + rect.width / 2;
      const fruitCenterY = rect.top + rect.height / 2;
      const dist = Math.hypot(mouseX - fruitCenterX, mouseY - fruitCenterY);
      if (dist < threshold) {
        fruit.dataset.runaway = 'true';
        // Pick a random spot in the viewport, avoid edges
        const vw = window.innerWidth;
        const vh = window.innerHeight;
        const randLeft = Math.random() * (vw - fruitSize * 2) + fruitSize;
        const randTop = Math.random() * (vh - fruitSize * 2) + fruitSize;
        fruit.style.left = `${(randLeft / vw) * 100}vw`;
        fruit.style.top = `${(randTop / vh) * 100}vh`;
        fruit.style.transform = 'scale(1.25)';
        setTimeout(() => {
          fruit.style.transform = '';
          fruit.dataset.runaway = '';
        }, 300);
      }
    });
  });
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  accents.forEach((accent, i) => {
    setTimeout(() => {
      accent.classList.add('animated-in');
      // After entrance, start floating animation
      setTimeout(() => {
        accent.classList.add('floating');
      }, 1200); // After entrance animation
    }, i * 1000); // Stagger each by 180ms (adjust for desired effect)
  });
});
